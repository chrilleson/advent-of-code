...
public static class Day8
{
    ...
    {
        var map = File.OpenText(@$"{AppContext.BaseDirectory}\inputs\day8.txt").ReadMap();
    ...
    private static IEnumerable<Position> NonResonatingAntiNodes(char[][] map, Position antenna, int rowDiff, int colDiff)
    {
        var position = new Position(antenna.Row + rowDiff, antenna.Column + colDiff);
        if (map.IsInside(position)) yield return position;
    }

    private static IEnumerable<Position> ResonatingAntiNodes(char[][] map, Position antenna, int rowDiff, int colDiff)
    {
    ...
    private static IEnumerable<Position> GetAntiNodes(this AntennaSet antennaSet, char[][] map, Func<char[][], Position, int, int, IEnumerable<Position>> foo) =>
