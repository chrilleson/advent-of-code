...
var calibrations = File.OpenText(@"..\..\..\input.txt").ReadCalibrations().ToList();
...
{
    public static IEnumerable<(long target, long first, IEnumerable<long> values)> ReadCalibrations(this TextReader reader) =>
        reader
            .ReadLines()
            .Select(x => x.ParseLongs())
            .Select(x => (x.First(), x.Skip(1).First(), x.Skip(2)));

    public static bool CanCreateTarget(this (long target, long first, IEnumerable<long> values) calibration, params Func<long, long, long, IEnumerable<long>>[] operations)
    {
